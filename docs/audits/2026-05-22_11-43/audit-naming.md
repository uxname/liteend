...
|----------|----------|--------|-------------|----------------|---------|------------|
| NAM-01 | Соглашение об именовании соблюдается консистентно (camelCase/snake_case) | ❌ FAIL 🟢 | High | `src/modules/file-upload/file-upload.service.ts:17-19` | 1. **Переименовать `UPLOAD_DIR` → `uploadDir`, `DEFAULT_MIME_TYPE` → `defaultMimeType`, `ALLOWED_MIME_TYPES` → `allowedMimeTypes`** \\ 2. Оставить как есть — приватные поля класса, нейминг не влияет на API \\ 3. Вынести в module-level константы, если они не зависят от экземпляра | Нет |
