...
| **VAL-08** | Загрузка файлов: MIME тип проверяется по содержимому, имя файла санитизировано, размер ограничен | ❌ FAIL 🟡 | Medium | `src/modules/file-upload/file-upload.service.ts:58` — MIME тип проверяется из `part.mimetype` (client-reported), а не по содержимому файла; разрешены SVG (`image/svg+xml`) — потенциальный XSS-вектор | **1. Проверять MIME тип по magic bytes (file-type или аналог)**; 2. Исключить SVG из разрешённых MIME типов или добавить санитизацию; 3. Усилить проверку размера на уровне `processFile` (дублирующий check) | Нет |
