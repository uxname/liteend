...
| CON-03 | HTTP статус-коды соответствуют семантике операции (201 создание, 204 удаление) | FAIL | HIGH | **`POST /upload` (file-upload.controller.ts:29)**: возвращает `200 OK` при успешном создании файла — должен возвращать `201 Created`. Остальные: `GET /uploads/*` → 200 (OK), `GET /health` → 200 (OK), `GET /dev` → 200 (OK), `GET /logs/api/list` → 200 (OK). DELETE эндпоинтов нет. Error mapping в `AllExceptionsFilter` корректен. | Изменить статус `POST /upload` на `201 Created` с помощью `@HttpCode(HttpStatus.CREATED)`. Возвращать `Location` header с URL загруженного файла. | НЕТ |
